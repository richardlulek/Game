 ...
 *  pengarna i fler hus. Därefter förvaltar man i stället. */
const EARLY_STAGE_MAX = 6;
/** Övervärde som andel av vad huset kostat (pris + nedlagd capex) innan det
 *  är värt att sälja. Under det äter mäklarrabatten upp vinsten. */
...
interface Run {
  ...
  /** Sålda hus och realiserat övervärde. */
  sold: number;
...
function report(label: string, runs: Run[]): void {
  ...
  const soldTotal = runs.reduce((a, r) => a + r.sold, 0);
  if (soldTotal > 0)
    console.log(
      `FÖRSÄLJNINGAR   ${soldTotal} hus sålda (${(soldTotal / runs.length).toFixed(1)} per parti)` +
      `  realiserat övervärde ${msek(runs.reduce((a, r) => a + r.realisedUplift, 0) / runs.length)} MSEK/parti`,
    );
  console.log("");

  console.log("frö    EK MSEK   värde MSEK   hyra MSEK/år   hus   sålda   slut-LTV   utfall");
  for (const r of runs) {
    console.log(
  ...
  console.log("    " + Object.entries(total).sort((a, b) => b[1] - a[1]).map(([k, v]) => `${k} ${v}`).join(" · "));
  console.log("");
}

suite("50 år på 55 % belåning med aktiv direktör", () => {
  it(`${SEEDS.length} frön · med och utan omsättning av beståndet`, () => {
    console.log(`\n${YEARS} år · ${SEEDS.length} frön · mål-LTV ${Math.round(TARGET_LTV * 100)} % · direktör med renoveringsprogram`);
    report("FÖRVALTAR – köper och behåller", SEEDS.map((sd) => play(sd, YEARS, false)));
    report("OMSÄTTER – säljer tidigt för att frigöra övervärdet", SEEDS.map((sd) => play(sd, YEARS, true)));
  }, 1_800_000);
});
