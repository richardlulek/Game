    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ fontSize: 11, letterSpacing: 6, color: "#6a4a00", fontWeight: 600, marginBottom: 4 }}>
          EST. 1925 · DAGSTIDNING FÖR FASTIGHETSMARKNADEN
        </div>
        ...
        <div style={{ fontSize: 12, color: "#6a4a00", marginTop: 4, display: "flex", justifyContent: "center", gap: 20 }}>
          ...
          <span>|</span>
          <span>Portfölj: {state.portfolio.length} objekt</span>
        </div>
      </div>
      ...
      {state.marketCycle && state.marketCycle.phase !== "stable" && (
        <div style={{
          ...
          {state.marketCycle.phase === "boom" ? "📈 HÖGKONJUNKTUR" : "📉 LÅGKONJUNKTUR"} — {state.marketCycle.monthsRemaining} månader kvar
        </div>
      )}
      ...
      {recent.length === 0 && (
        <div style={{ textAlign: "center", padding: 40, color: "#888" }}>
          Inget att rapportera ännu. Starta spelet och läs nyhetsflödet här.
        </div>
      )}
    </div>
  );
