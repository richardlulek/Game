/* ============================================================
   ...
   2. Hyresgästen    – Gus-dilemmat, ansökningar, kontrakt
...
export const GOSTA_NAME = "Gus (Grandpa's buddy)";
export const ROGGE = "Roger ”Rog” Flint";
...
export const STORY_BEATS: StoryBeat[] = [
  ...
  },
  {
    ...
    objectives: [
      {
        text: "Win the bidding war against Rog over the neighboring house",
        check: (s) => s.portfolio.some((p) => p.storyTag === "revansch"),
      },
    ],
    hint: "The neighboring house is in the Market window. Rog bids against you – outbid him.",
  },
  {
    id: "dynastin",
    chapter: 8,
...
const STORY_DECISIONS: Record<string, StoryDecisionFactory> = {
  ...
  brev_morfar_1: () => ({
    ...
    text:
      "”If you're reading this, I'm dead. Or Oakley sent the letter too early again – in which case, see you Sunday as usual. " +
      ...
      "Gus lives on the ground floor. He doesn't pay rent, but he guards the house. Against what has never become clear. " +
    ...
    options: [
      {
        ...
        detail: "Reputation +1 · Rog won't forget you",
        effect: { reputation: 1, storyFlag: "prolog_läst", log: "Rejected Rog's lowball bid. He smiled like a man who has seen pictures of smiles.", logKind: "event" },
      },
      {
  ...
  brev_kap1: () => ({
    ...
    text:
      ...
      "Take the money from the bag and FIX UP the house. Condition below 40 scares off every tenant except Gus, and Gus doesn't count, he's Gus. " +
  ...
  gosta_dilemma: () => ({
    ...
    options: [
      ...
      {
        label: "”I need the unit, Gus.”",
        detail: "Gus moves out · the unit is freed · Reputation −2",
        effect: {
          reputation: -2, storyFlag: "gosta_ut", nextDecisionId: "brev_kap2",
          log: "Gus packed the thermos and moved to his sister in The Suburbs. The house feels strangely quiet. And unguarded.", logKind: "warn",
        },
      },
    ],
  }),
  ...
  brev_kap4: () => ({
    id: "story:brev_kap4",
    portrait: "ulla",
    title: "🏦 Meeting: Ruth at Oak Savings Bank",
    text:
  ...
  brev_kap6: () => ({
    ...
    text:
      ...
      "I tried to register a company once. 'House Ltd' – taken. 'House 2 Ltd' – taken. 'Gordon's Houses & Revenge Ltd' – approved, but Grandma said no. " +
  ...
  brev_kap7: () => ({
    id: "story:brev_kap7",
    portrait: "rogge",
    title: "🕶️ Rog Flint buys up your street",
    text:
      "Rog Flint stands outside the neighboring house photographing it with a thumbs-up. " +
      "”What's up, heir! Flint Properties is expanding. I'm buying the shack next door, ripping out everything with SOUL in it and renting per square millimeter. " +
      "Then I take the next house. And the next. Soon the hill will be called Flint Hills. It's in focus groups right now.” " +
      "The neighboring house has just been listed for sale – and Rog has already placed a bid. Grandpa would have expected you to do something about it.",
    options: [
      {
        label: "Not my street, Rog.",
        detail: "Goal: win the bidding war over the neighboring house",
        effect: { log: "Chapter 7: The Rematch. Rog Flint is bidding on the neighboring house – outbid him on the Market.", logKind: "event" },
      },
    ],
  }),
  ...
  rogge_surbrev: () => ({
    ...
    text:
      ...
      "Flint Properties is now PIVOTING to more exciting verticals. Padel. Padel is the future. " +
      "See you at the top – I'll take the elevator, you get the stairs. /R Flint, CEO, founder, visionary, driver's license” " +
      "PS. Oakley sends word that Grandpa foresaw this letter and asked him to attach: ”Ha. — Gordon”",
    options: [
      {
  ...
  brev_epilog: () => ({
    ...
    text:
      "”If Oakley has done his job, you get this letter when you no longer need it – when the house has become many, " +
...
export function makeInheritedHouse(fresh: GameState): Property {
  ...
  return {
    ...
    storyTag: "arvet",
    txHistory: [{ type: "bought", price: 0, month: fresh.month, year: fresh.year, party: "Inheritance from Grandpa Gordon" }],
  };
}
...
/** Grannhuset i kapitel 7 – Rog bjuder direkt. */
    ...
    case "konjunkturen": {
      ...
      return {
        ...
        story: s.story
          ...
          : s.story,
        log: [{ t: `🏦 THE CENTRAL BANK raises the policy rate by ${hike.toFixed(2)} percentage points. Ruth at the bank called just to say ”what did I tell you”.`, kind: "warn" as const }, ...s.log],
      };
    }
    case "revanschen": {
...
export function advanceStory(state: GameState): GameState {
  ...
  for (let guard = 0; guard < 4; guard++) {
    ...
    // 3) Kapitel 7: om Rog vann huset – lägg ut det igen (han flippar det).
    if (beat.id === "revanschen") {
      ...
      if (!stillListed && !owned) {
        ...
        s = {
          ...
          log: [{ t: "🕶️ Rog flips the neighboring house – back on the market, now with a ”styled entryway” and a higher price.", kind: "warn" as const }, ...s.log],
        };
        return s;
      }
    }
...
export const CHAPTER_FRONTS: Record<string, StoryFront> = {
  renoveringen: {
    ...
    sub: "The neighbors: ”Finally.” Gus: ”The kitchen crackles less now. Suspicious.”",
    body:
      "Villa Hill's most talked-about property – colloquially 'Gordon's shack' – has been renovated by the heir. " +
  ...
  banken: {
    ...
    sub: "Oak Savings Bank: ”The math holds. We counted twice. Ruth three times.”",
    body:
  ...
  revanschen: {
    ...
    sub: "Roger Flint: ”Entirely voluntary. Real estate was really mostly a thing for people who like houses.”",
    body:
      "The bidding war over the neighboring house on Villa Hill is over: the heir won. Flint Properties announces " +
 ...
 * ägas av de skriptade sökandena (metalbandet, Marge och influencer-
...
export const STORY_CINEMATICS: Record<string, StoryCinematic> = {
  ...
  // Kap 7: den vita sedanen glider in vid grannhuset. Rog är tillbaka.
  "story:brev_kap7": {
    focusTag: "revansch", zoom: 62, holdMs: 4200, car: true,
    hint: "A familiar white sedan parks by the neighboring house …",
  },
  // Kap 8: sista distriktet öppnar – och Rog fattar pennan i vredesmod.
  "story:rogge_surbrev": {
