/* ============================================================
   ...
   att en dold flik inte ger "catch-up" när spelaren kommer
   tillbaka (rAF pausar när fliken är dold).
   ============================================================ */
...
export function useGameClock(): void {
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      acc += dt * speed;
      let ticked = false;
      while (acc >= MONTH_MS && !useGameStore.getState().state.gameOver) {
        acc -= MONTH_MS;
      ...
      if (ticked) {
        ...
        if (store.state.gameOver) store.setRunning(false);
      }
