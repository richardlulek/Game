...
export function frontageState(p: Property) {
  const working = !!p.phased || !!p.renovation || (p.pendingWorks?.some(w =>
    w.kind === "underhåll" || w.kind === "energi" || (w.kind === "uppgradering" && ["fasad", "renovering", "tillbygg", "energi"].includes(w.upgradeId ?? ""))) ?? false);
  ...
  return { working, occupancy, open, vacant: p.status === "klar" && occupancy === 0,
    ...
    label: working ? "WORK IN PROGRESS" : !open ? "TO LET" : p.type === "bostad" ? "" : p.tenants[0]?.name ?? "OPEN" };
}
...
export function frontageParts(width: number, district: string, p: Property, low = false): FrontageParts {
  ...
  if (p.type === "industri") {
    ...
    if (room > 0.7) for (const side of [-1, 1]) {
      ...
      if (state.worn && !low) for (const y of [0.85, 1.5]) add("wood", x, y, 0.69, bw * 0.94, 0.2, 0.08);
    }
  }
